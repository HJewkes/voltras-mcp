...
- `accountability.state` reports the coach protocol's position and the decision it would
  make right now, read-only (VW-286). It answers "why has the coach been quiet" with a
  ...
  in-memory implementation, and the schema gains an `accountability_state` table (v19).
