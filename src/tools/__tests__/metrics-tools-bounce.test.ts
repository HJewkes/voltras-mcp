// `metrics.compute quality.bounce` — VMCP-06.11 / B10 wiring.
//
// The detector itself (`detectBounce`) is unit-tested in
// `src/analytics/__tests__/rep-faults-bounce.test.ts`; this file only pins
// the tool-layer plumbing: fetch → map every rep → set summary, and EC-07's
...
describe('metrics.compute — quality.bounce', () => {
  it('fetches the set and returns a bounce reading per rep with null counts', async () => {
    ...
    const result = await callTool(tools, { pipeline: 'quality.bounce', setId: 'set-1' });
      ...
      reps: [
        {
          ...
          dwellLengthenedMs: 0,
          dwellShortenedMs: 0,
          eccentricPeakOverConcentricPeak: 1,
          bounce: null,
          diveBomb: null,
        },
        {
  ...
  it('EC-07: returns NOT_FOUND for a missing set', async () => {
    ...
    const result = await callTool(tools, { pipeline: 'quality.bounce', setId: 'missing' });
