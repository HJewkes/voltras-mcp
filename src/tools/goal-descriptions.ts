...
export const GOAL_WEEKLY_REVIEW_DESCRIPTION =
  ...
  'committed goal line, and whether that is worth acting on. Takes `weekOf` (a date, defaults ' +
  'to the most recent Sunday, the same anchor `profile.log_weekly_checkin` uses) and an optional ' +
  '`response`. EVERY INPUT IS READ, NONE IS TYPED — the weight series, the declared diet phase ' +
  'and that week’s check-in. Returns `observation` (the observed percent per week, the band it ' +
  'is judged against, how far off the line and which way the gap is moving), `advisory`, ' +
  '`levers`, `vetoes`, `offCadenceConditions`, `confounders`, `lowConfidence`, `checkin`, ' +
  '`readingCount`, `targetId`, `committedValue`, `stretchValue`, `committedUnchanged`, ' +
  '`outcome`, `proposal`, `suppressedByDecline`, `reviewedAt` and `notes`. ' +
  'THE ADVISORY IS NEVER SIZED: it names the observation and the two levers, intake and ' +
  'activity, and says which one to move is the lifter’s pick ' +
  '(rp:rp-s12-activity-vs-food-adjustment-choice). This server prescribes no calories and no ' +
  'macros. A week under the weekly noise floor, still inside the settling window after a phase ' +
  'change, holding a down-weighted single-day jump, or flat late in a cut with adherence ' +
  'reported high is VETOED: nothing is proposed and `vetoes` says which rule held it ' +
  '(rp:rp-s12-no-adjustment-under-half-pound-weekly-change, ' +
  'rp:rp-s12-scale-opacity-salt-and-water). ' +
  'NOTHING ON THE CHART MOVES: the accepted target’s committed and stretch values are read, ' +
