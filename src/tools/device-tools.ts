...
const SET_ECCENTRIC_DESCRIPTION =
  ...
  'COST FRAMING (VW-303): a positive `overloadLbs` is a stimulus-cost knob, not a growth multiplier — the largest available synthesis found chronic strength and hypertrophy adaptations statistically similar to constant-load training, while acutely raising lactate, growth hormone, RPE and eccentric-phase muscle activation for the same result (see `coaching.explain { topic: "live.eccentric_overload_cost" }` for the full citation and caveats).';
...
export function registerDeviceTools(
    ...
    wrapHandler(DeviceUnloadInput, async (input) => {
      ...
      return {
        ...
        read_back: readBackUnload({
          slotId,
          phase: slot.client.guidedLoadState.phase,
          loadState: deriveLoadState(
  ...
  install(
    ...
    wrapHandler(DeviceStartGuidedLoadInput, async (input) => {
      ...
      // VW-168a: same single-shot stash for the set-level intent. The bridge
      // mints the set on `armed`, before any `set.start` could attach these.
      if (input.isWarmup === true) {
      ...
      // VMCP-02.88: the trigger is written; now look. `ok` alone only ever
      // said the write did not throw, which on a load-applying device is not
      // the same claim as "the flow started".
      return {
        ok: true,
        read_back: await readBackGuidedLoadTrigger({
          slotId: guidedSlotId,
          targetWeightLbs: input.targetWeightLbs,
          readPhase: () => slot.client.guidedLoadState.phase,
          checkFence: () => guidedFence.check(guidedSlotId),
        }),
      };
    }),
