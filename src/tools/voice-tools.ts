...
const STOP_DESCRIPTION = [
  ...
  '',
  'During TTS the mic is safety-only. `system.speak` and the automatic cue',
  'emitter duck the listener for the length of each cue, so the wake phrase and',
  'spoken weight commands are ignored until playback ends (an utterance already',
  'in progress when the cue starts is dropped too), bounded by a hard 8s',
  'failsafe. Safety phrases (stop, unload, cut the weight, …) stay live through',
  'that window and still fire the unload — except a transcript made of the words',
  'being spoken, which is discarded as the machine hearing itself, so a lifter',
  'shouting a safety word that also appears in the cue text is dropped with it.',
  'Call this tool directly rather than waiting to be told to.',
].join(' ');
