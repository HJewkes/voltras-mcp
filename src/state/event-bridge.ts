...
export function wireBridgeForSlot(state: ServerState, slot: SlotState): () => void {
  ...
  // Firmware-rep dedupe key. The SDK fires `onPerRep` twice per real rep
  ...
  pushUnsub(
    ...
    client.onFrame((frame: TelemetryFrame) => {
      ...
      const sample: WorkoutSample = {
        ...
        // Single mm→m conversion point (WA 2.0.0). WA's `WorkoutSample.position`
        // contract is cable extension in metres, converted at the producer's
        // bridge — WA itself performs no conversion and validates nothing at
        // runtime. `frame.position` is the device-native 0-600 (mm) cable
        // extension; converting here means every downstream position-derived
  ...
  pushUnsub(
    ...
    client.onPerRep((payload: PerRepEvent) => {
      ...
      debug.events.push({
        ...
        payload: {
          ...firmwareRep,
          firmwareRepsSoFar: live.snapshotSet()?.firmwareReps?.length ?? 0,
        },
      });
    }),
  );
