...
The 16-task VMCP-01 plan is complete and all 186 unit + integration tests pass. Three production blockers were surfaced and fixed during the build-out (placeholder schema, two-client divergence on `device.connect`, Rep assembly from frame stream). Four follow-ups remain — all are either upstream package work or product decisions:

| #   | Issue                                                                                                                                                                                                                                                                                                                 | Fix Path                                                                                                                                                                                                                                                                                 |
| --- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | `@voltras/node-sdk@0.3.0`'s ESM build has bare extensionless imports + a CJS `require()` in `forMock`, so it can't load under stock Node ESM or vite/vitest. Tests stub the SDK via `vi.mock`.                                                                                                                        | UPSTREAM in `voltra-node-sdk`: add `tsc-alias` (or equivalent) to append `.js` extensions, drop `require()` from `forMock`, ship a nested `package.json` with `"type": "module"` in `dist/esm/`.                                                                                         |
...
| 3   | `MockBLEAdapter.configure()` and `MockBLEAdapter.injectError()` are not on the public SDK surface in 0.3.x. `mock.configure` / `mock.inject_error` therefore return `NOT_IMPLEMENTED`.                                                                                                                                | UPSTREAM in `voltra-node-sdk`: add `configure(config: MockBLEConfig)` and `injectError(category)` as public methods on `MockBLEAdapter`, plus a `getAdapter()` accessor on `VoltraManager`.                                                                                              |
| 4   | `metrics.compute` returns `NOT_IMPLEMENTED` for `quality.rep` and `session.readiness`. `assessRepQuality(rep, baseline)` requires a `TechniqueBaseline` not in the input schema; `computeReadiness(actualVel, baselineVel)` requires two scalars not derivable from a session id alone. The other six pipelines work. | Schema extension: add `baselineSetId: IdSchema` to `quality.rep` (handler builds baseline via `createTechniqueBaseline`); add `baselineSessionId: IdSchema` to `session.readiness` (handler reads first-rep mean velocity from each session). Both are product decisions awaiting input. |

`mock.*` and `metrics.{quality.rep,session.readiness}` not-implemented states return structured error results with a clear `code: 'NOT_IMPLEMENTED'` and a one-line explanation pointing at the missing input. They do not throw; clients handle them as a normal MCP error response.
