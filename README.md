...
| `VMCP_AUTO_ARM`           | `on`                            | `off` \| `on`                          | When `on`, the first idle rep detected while a session is open (and no set is) opens a set on the server's own initiative and counts that rep as its rep 1. Reps begin within ~1s of a weight change on the unit, so the gap before a `set.start` lands costs real reps. `off` restores idle-rep reporting only. `server.health` reports the live value.                                                                                                                               |
