...
| `VMCP_CUES_MIDSET`             | `off`                             | `off` \| `on`                          | Whether the two cue categories that fire while the lifter is still under load (`target_hit`, `slowdown`) may speak. Every cue ducks the mic for its duration; since VMCP-05.20 the voice "stop" path stays live through that window while the wake phrase and weight commands do not ([design + residual risk](docs/vmcp-05.20-safety-during-cues.md)). Still off by default until that exemption is verified on hardware. Also a startup default only — see `system.set_cues`.                                                 |
...
| `VMCP_REP_UNRACK_DROP`         | `off`                             | `off` \| `on`                          | Drop the un-rack artifact rep at set close. Dark until validated across movement classes — on an untested movement it can drop a valid rep, and it changes the persisted rep count.                                                                                                                                                                                                                                                                                                                                             |
| `VMCP_REP_ECC_TRUNCATE`        | `on`                              | `off` \| `on`                          | Truncate the final rep's parked idle tail off its eccentric at set close, so that rep's `tempo_ratio` and eccentric mean velocity stop counting time the lifter was not moving. Deletes no rep and rewrites no raw sample.                                                                                                                                                                                                                                                                                                      |
| `VMCP_REP_CORRECTIONS`         | _unset_                           | `off` \| `on`                          | Legacy coarse switch over both of the above. Unset, each of them uses its own default; set, it moves both together. Either one's own variable overrides it.                                                                                                                                                                                                                                                                                                                                                                     |
...
## Backing up the training store

`npm run store` exports the SQLite store to plain text, rebuilds a store from that text,
and proves the two match. It is the backup, the exit, and the first half of any later move
...
process first, so the copy is not taken mid-write.
...
`.store-scratch/` is gitignored. An export of the real store holds the whole training
...
that. The loader only ever inserts, which is how it honours the store's append-only tables
instead of switching their guards off, and foreign keys stay enforced (deferred to the end
of the one transaction the load runs in).
...
98 tools in mock mode; 96 with the real adapter (`mock.*` is registered only when
...
| `profile.*`     | 6     | Self-reported training background (get/set), the derived tier signal, `get_starting_prescription` — conservative tier-seeded sessions/week, sets/exercise and RIR target — `get_onboarding_gaps`, and `set_diet_phase` (the OBSERVED fat-loss/gain/maintenance range, retroactively correctable). Advisory: they apply nothing.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| `exercise.*`    | 3     | Search and fetch from the exercise catalog, plus `confirm_setup` (VW-119) — give an inferred physical setup the name the lifter uses for it ("bench at 30 degrees"). The clustering can tell that two groups of sets moved the cable different distances; only a human knows what the difference was, so this is the only writer of a real label. Ask, never guess.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
| `baselines.*`   | 2     | Per-exercise baseline confidence STATE (`get`, `recalc`) — never baseline values, which are recomputed from stored reps on demand. Both take an optional `setupId` to key the baseline to one inferred physical setup rather than the pooled row; `recalc { inferSetups: true }` re-runs the ROM clustering that produces those ids (VW-119) and returns the setups it found. The split rule is a relative ratio of median range of motion, never an absolute distance.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
| `metrics.*`     | 1     | `compute` — runs an analytics pipeline over a session, a set, or a set-id array, including `strength.e1rm` (VW-142) — Epley/profile/hybrid 1RM estimates from `{ load, reps }`, `{ exerciseId }`, or both — `history.trend` (VW-144/VW-145) — per-exercise `{ series, trend, plateau }` over a weekly-bucketed window via WA's `buildTimeSeries`/`analyzeTrend`/`detectPlateau`, with the trend's `direction` always `null` (VW-230) — every metric here is a load and no citable up/down/flat band exists for a load trend, so the raw fit ships instead: `slope` is the per-day change in `slopeUnit`, beside `rSquared`, `percentChange`, `windowDays` and `confidence`, and `directionReason` says in words why no verdict is given — `plateau.phase` always `'unknown'` pending diet-phase tagging (VW-149) — and `quality.rom` (VW-93/B09) — within-set ROM decay and rep-to-rep variance for one set, verdicts cut by WA's own `DEFAULT_PARTIAL_REP_SCHEME`/`DEFAULT_CONSISTENCY_SCHEME` and cited in the response; no population or absolute ROM figure is involved, and the one cross-session number (`baseline.romVsBaselinePct`) stays `null` unless the B57 baseline is at least PROVISIONAL, and B15's `checkDriftGuard` calls this set's session comparable to the reference session. Rows from different capture eras (`positionUnits`) compare normally: `src/store/position-units.ts` maps both sides to metres on read (VW-203), the position sibling of the `velocity-units.ts` normaliser. `history.weekly_volume` (VW-144/VW-145/VW-201) rolls up weekly totals plus a per-muscle-group breakdown across EVERY exercise via WA's `getWeeklySummaries`/`getVolumeByMuscleGroup`, target-only attributed to each exercise's primary muscle group (B47, matching `session.volume`'s `setsByTargetMuscle`); `verdict` is always `null` — `classifyWeeklyVolume` needs caller-supplied `VolumeLandmarks` this repo states nowhere for this athlete. The one local analytics module family is `src/analytics/` — `rep-faults.ts` (`quality.hesitation` #282, `quality.bounce` #285), `rom-integrity.ts` (`quality.rom` #293), and `comparability.ts` (the B16 like-vs-like predicate #295) — kept local because each is a unit-free, relative-to-the-rep readout over persisted samples that WA does not ship; every other pipeline dispatches straight to `@voltras/workout-analytics`. `session.readiness` and `session.strength` also carry the `comparability` block described in [Like-vs-like comparisons](#like-vs-like-comparisons). |
| `progression.*` | 1     | Progression history for one exercise, optionally filtered to one `side` (VMCP-04.09) — omitted adds a `sideSplit` per-arm summary instead. Also carries the `comparability` block described in [Like-vs-like comparisons](#like-vs-like-comparisons).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
Every "you got stronger" claim is the inference _same work, different result_, and it is
only sound while the work really was the same. `src/analytics/comparability.ts` (VW-94)
is the one predicate that answers that question about two stored sets: same exercise,
same lifter, same set purpose, same training mode, same device-settings hash, same side,
same load, plus a training-phase and a physical-setup clause.

Three tools use it to CHOOSE what to compare against and report the choice in a
`comparability` block: `metrics.compute` with `session.readiness` (which baseline set)
and `session.strength` (whether the pooled sets match the heaviest one), and
`progression.get_for_exercise` (which earlier session is a valid basis).

Two rules keep it honest:

- **Exact load, no invented tolerance.** The RP source names no load tolerance for this
  comparison, so loads must match exactly. A percentage nobody measured would sit at the
  centre of every progress claim.
- **It never answers with silence.** Strict matching leaves an irregular lifter with few
  valid pairs, so a block with no match still names the nearest candidate and the reasons
  it failed (`noValidComparison` + `nearest`), and the pipeline still returns its numbers.
  Relay those reasons rather than the headline alone.

This is a check on CONTEXT. Whether two sets were _executed_ the same way — tempo and ROM
...
npm run store -- export ...          # export / import / verify the training store
